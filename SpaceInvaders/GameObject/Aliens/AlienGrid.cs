...
namespace SpaceInvaders
{
    public class AlienGrid : Composite
    {
        ...
        private float gridStartLocation = 750.0f;
        private float gridStartSpeed = 0.5f;

        // Attached to SpriteBatches
        SpriteBatch pSB_Bombs;
        SpriteBatch pSB_Box;
        public AlienGrid(GameObject.Name name, GameSprite.Name spriteName, float posX, float posY, ScenePlay scene)
        ...
        public void ActivateBomb()
        {
            ...
            if (pNode != null)
            {
                ...
                {
                    case 0:
                        pBomb = new Bomb(GameObject.Name.Bomb, GameSprite.Name.PlungerShotA, new FallDagger(), pGameObject.x, pGameObject.y - 30f);
                        pBomb.ActivateCollisionSprite(pSB_Box);
                        pBomb.ActivateGameSprite(pSB_Bombs);
                        break;

                    case 1:
                        pBomb = new Bomb(GameObject.Name.Bomb, GameSprite.Name.RollingShotA, new FallStraight(), pGameObject.x, pGameObject.y - 30f);
                        pBomb.ActivateCollisionSprite(pSB_Box);
                        pBomb.ActivateGameSprite(pSB_Bombs);
                        break;
                    case 2:
                        pBomb = new Bomb(GameObject.Name.Bomb, GameSprite.Name.SquigglyShotA, new FallZigZag(), pGameObject.x, pGameObject.y - 30f);
                        pBomb.ActivateCollisionSprite(pSB_Box);
                        pBomb.ActivateGameSprite(pSB_Bombs);
                        break;
                    default:
        ...
        public override void Delete()
        {
            ...
            TimeEvent pSpawn = TimerManager.Search(TimeEvent.Name.BombSpawn);
            TimerManager.Delete(pSpawn);

            TimeEvent pSpriteAnim = TimerManager.Search(TimeEvent.Name.SpriteAnimation);
            while (pSpriteAnim != null)
            {
                TimerManager.Delete(pSpriteAnim);
                pSpriteAnim = TimerManager.Search(TimeEvent.Name.SpriteAnimation);
            }
